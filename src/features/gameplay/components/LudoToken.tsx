...
/**
 * Premium 3D SVG token matching the Snake & Ladders style.
...
export const LudoToken: React.FC<LudoTokenProps> = ({
  ...
  const palette = {
    ...
    YELLOW: {
      ...
      badge:  "#ffab00",
      label:  "Y",
      shadow: "rgba(255,171,0,0.85)",
    },
  };

  const p = palette[color];
  const gradId  = `ludo-body-${color}-${tokenIndex}`;
  const grad2Id = `ludo-base-${color}-${tokenIndex}`;
