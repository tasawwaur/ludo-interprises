...
import { OwnedDice } from '../components/Inventory/OwnedDice';

interface DiceInventoryPageProps {
  onBack?: () => void;
}

export const DiceInventoryPage: React.FC<DiceInventoryPageProps> = ({ onBack }) => {
  const { diceItems, equippedDiceId, equipDice } = useDice();
  const user = useUserStore((s) => s.user);

  const ownedItems = diceItems.filter((d) => d.isUnlocked);
...
export default DiceInventoryPage;
